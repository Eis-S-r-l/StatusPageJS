...
function IncidentForm({ item, services, onSuccess }: { item?: IncidentItem; services: ServiceOption[]; onSuccess?: () => void }) {
  ...
  return <form action={formAction} className={styles.form}>
    ...
    <AutoSlugFields
      ...
      beforeSlug={<label className={styles.field}>Status<select name="status" defaultValue={stateValue(state, "status", item?.status ?? "investigating")}><option value="investigating">Investigating</option><option value="identified">Identified</option><option value="monitoring">Monitoring</option><option value="resolved">Resolved</option></select>{item?.isPublished && <span className={styles.fieldHint}>Manual failover only: this changes the current public status without adding a timeline update or notifying subscribers.</span>}</label>}
    />
